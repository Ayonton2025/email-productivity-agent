...
const parseLineForLinks = (line) => {
  ...
  const labelUrl = trimmed.match(
    /^(view image|follow image link|link|url)\s*:\s*(https?:\/\/\S+)$/i
  );
  if (labelUrl) {
    ...
    return [
      { type: 'text', content: `${label}: ` },
      { type: 'link', text: shortenUrl(url), url },
    ];
  }
...
export const EmailBodyRenderer = ({ bodyText, className = '' }) => {
  ...
  return (
    <div
      className={`whitespace-pre-wrap font-sans text-slate-800 text-[15px] leading-relaxed ${className}`}
    >
          ...
          return (
            <p key={paragraph.key} className="mb-2">
              {paragraph.content.map((item, idx) => {
                if (item.type === 'text') {
                  return <span key={`${paragraph.key}-text-${idx}`}>{item.content}</span>;
                }
