...
vi.mock('../components/inbox/AttachmentsSection', () => ({ default: () => <div>Attachments</div> }))
  ...
  it('sends the generated reply with thread metadata', async () => {
    ...
    await waitFor(() =>
      expect(emailApi.sendEmail).toHaveBeenCalledWith(
        'account-1',
        expect.objectContaining({ to: 'customer@example.com', subject: 'Re: Need help', body_text: 'Reply body' })
      )
    )
