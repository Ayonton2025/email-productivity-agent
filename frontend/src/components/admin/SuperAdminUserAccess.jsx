        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 text-sm text-slate-800 md:grid-cols-2">
              ...
              <div>
                <span className="text-slate-500">Plan:</span> {data.subscription?.plan_name} (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  <div className="text-slate-800">
                    Campaigns:
                    <span
                      className={`ml-2 rounded px-2 py-1 text-xs font-semibold ${sendReadiness?.overall?.campaign_ready ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}
                    >
                  ...
                  <div className="text-slate-800">
                    Direct/Reply Send:
                    <span
                      className={`ml-2 rounded px-2 py-1 text-xs font-semibold ${sendReadiness?.overall?.reply_ready ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}
                    >
                  ...
                  {(sendReadiness?.accounts || []).map((acc) => (
                    <div key={acc.id} className="rounded border border-slate-200 p-2">
                      <div className="flex flex-wrap items-center gap-2 text-sm text-slate-900">
                        ...
                        <span
                          className={`rounded px-2 py-0.5 text-xs font-semibold ${acc.reply_send_ready ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}
                        >
