      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Monitor sending health, spam risk, and remediation actions.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <MetricCard
              label="Bounce Rate"
              value={`${(data.metrics?.bounce_rate * 100).toFixed(2)}%`}
            />
            <MetricCard
              label="Block Rate"
              value={`${(data.metrics?.hosted_block_rate * 100).toFixed(2)}%`}
            />
            <MetricCard
              label="Open Rate"
              value={`${(data.metrics?.open_rate * 100).toFixed(2)}%`}
            />
            <MetricCard
              label="Reply Rate"
              value={`${(data.metrics?.reply_rate * 100).toFixed(2)}%`}
            />
