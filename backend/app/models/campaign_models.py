...
class Campaign(Base):
    ...
    __table_args__ = (Index("idx_campaigns_user_status", "user_id", "status"),)
...
class CampaignSequence(Base):
    ...
    __table_args__ = (Index("idx_campaign_order", "campaign_id", "step_order"),)
...
class Lead(Base):
    ...
    )  # pending, sent, delivered, opened, clicked, replied, bounced, unsubscribed, completed
...
class WarmupSchedule(Base):
    ...
    __table_args__ = (Index("idx_warmup_campaign_day", "campaign_id", "day_of_week"),)
