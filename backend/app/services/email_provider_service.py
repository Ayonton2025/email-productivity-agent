...
class EmailProviderService:
    ...
    async def authenticate_gmail_with_token(self, access_token: str, refresh_token: str = None) -> bool:
        ...
        try:
            ...
            print(f"🔐 [EmailProviderService] Refresh token provided: {bool(refresh_token)}")
            ...
            print("🔐 [EmailProviderService] Building Gmail service...")
            ...
            print("🔐 [EmailProviderService] Validating credentials with API call...")
            try:
                # Make a simple API call to verify credentials work
                profile = self.gmail_service.users().getProfile(userId='me').execute()
                print(f"✅ [EmailProviderService] Gmail authentication successful, email: {profile.get('emailAddress')}")
                return True
            except Exception as auth_error:
                # If the token is expired and we can't refresh it, that's still an auth failure
                print(f"❌ [EmailProviderService] API validation failed: {type(auth_error).__name__}: {auth_error}")
                # Check if it's a refresh token error
                if "refresh_token" in str(auth_error).lower() or "refresh" in str(auth_error).lower():
                    print("⚠️ [EmailProviderService] Token needs refresh but no client credentials provided")
                    print("ℹ️ [EmailProviderService] This is expected for tokens from OAuth playground or third-party apps")
                    ...
                    try:
                        ...
                        profile = self.gmail_service.users().getProfile(userId='me').execute()
                        print(f"✅ [EmailProviderService] Gmail authentication successful (access token only), email: {profile.get('emailAddress')}")
                        return True
                    except Exception as fallback_error:
                        print(f"❌ [EmailProviderService] Fallback authentication also failed: {fallback_error}")
                        return False
                return False
