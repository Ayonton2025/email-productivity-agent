...
class EmailProviderService:
    ...
    async def authenticate_gmail_with_token(self, access_token: str, refresh_token: str = None) -> bool:
        ...
        try:
            logger.info(
                f"🔐 [EmailProviderService] Authenticating with access_token: {access_token[:20] if access_token else 'None'}..."
            )
            if not access_token:
                logger.info("❌ [EmailProviderService] Access token is empty!")
                return False

            logger.info(f"🔐 [EmailProviderService] Refresh token provided: {bool(refresh_token)}")

            # Create credentials with only the access token
            # If we need to refresh later, the caller should handle that
            creds = Credentials(
                token=access_token,
            ...
            logger.info("🔐 [EmailProviderService] Validating credentials with API call...")
            try:
                # Make a simple API call to verify credentials work
                profile = self.gmail_service.users().getProfile(userId="me").execute()
                logger.info(
                    f"✅ [EmailProviderService] Gmail authentication successful, email: {profile.get('emailAddress')}"
                )
                return True
            except Exception as auth_error:
                # If the token is expired and we can't refresh it, that's still an auth failure
                logger.error(
                ...
                if "refresh_token" in str(auth_error).lower() or "refresh" in str(auth_error).lower():
                    ...
                    # If we have just the access token, we can still try to use it as-is
                    # by creating credentials that won't auto-refresh
                    try:
                        # Try without refresh - some tokens are valid longer
                        ...
                        self.gmail_service = build("gmail", "v1", credentials=creds_no_refresh)
                        profile = self.gmail_service.users().getProfile(userId="me").execute()
                        logger.info(
                            f"✅ [EmailProviderService] Gmail authentication successful (access token only), email: {profile.get('emailAddress')}"
                        )
                        return True
    ...
    async def exchange_gmail_code(self, client_id: str, client_secret: str, code: str, redirect_uri: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "token_expiry": credentials.expiry,  # Return datetime object directly, not string
            ...
            traceback.print_exc()
            error_msg = f"Token exchange error: {type(e).__name__}: {e}"
    ...
    async def authenticate_gmail(self, credentials_file: str, token_file: str) -> bool:
        ...
        if os.path.exists(token_file):
            creds = Credentials.from_authorized_user_file(token_file, self.GMAIL_SCOPES)
        ...
        try:
            ...
            logger.error(f"Gmail authentication error: {error}")
            return False
    ...
    async def fetch_outlook_emails(self, max_results: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching Outlook emails: {error}")
            return []
    ...
    async def _send_outlook_reply(self, original_email_id: str, draft_content: Dict[str, str]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error sending Outlook reply: {error}")
            return False
    ...
    def _create_outlook_message(self, draft_content: Dict[str, str]) -> Dict:
        return {
            "message": {
                ...
                "body": {"contentType": "Text", "content": draft_content.get("body", "")},
                "toRecipients": [{"emailAddress": {"address": draft_content.get("recipient", "")}}],
            }
        }
