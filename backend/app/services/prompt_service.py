...
class PromptService:
    ...
    async def get_active_prompt(self, category: str) -> Optional[PromptTemplate]:
        ...
        # - Prefer system prompts (more curated)
        # - Then oldest created (stable across restarts)
        result = await self.db.execute(
            select(PromptTemplate)
            .where((PromptTemplate.category == category) & (PromptTemplate.is_active == True))
            .order_by(PromptTemplate.is_system.desc(), PromptTemplate.created_at.asc())
        )
    ...
    async def get_system_prompt(self, name: str = None) -> Optional[PromptTemplate]:
        ...
        try:
            query = select(PromptTemplate).where(
                (PromptTemplate.is_system == True) & (PromptTemplate.is_active == True)
            )
    ...
    async def update_prompt(
        ...
        try:
            ...
            logger.error(f"❌ [PromptService] Error updating prompt: {e}")
            raise
