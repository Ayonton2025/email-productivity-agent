...
class EnhancedLLMService:
    ...
    async def advanced_email_analysis(self, email_data: Dict[str, Any], analysis_type: str) -> Dict[str, Any]:
        ...
        if not self.client:
            logger.info("⚠️ [EnhancedLLMService] OpenAI client not initialized - using fallback")
    ...
    async def conversational_agent(self, messages: List[Dict[str, str]], email_context: List[Dict] = None) -> str:
        ...
        system_message = {
            "role": "system",
            "content": f"""
            You are Bylix Email, an intelligent Email Intelligence Platform assistant. You have access to the user's email context and can provide sophisticated operational insights.
        ...
        if not self.client:
            return "I apologize, but no LLM client is configured. Set provider API keys in Admin LLM settings."
    ...
    async def generate_smart_reply(
        self, original_email: Dict[str, Any], tone: str = "professional", context: List[Dict] = None
    ) -> Dict[str, Any]:
