...
class EmailService:
    ...
    async def generate_reply_draft(
        ...
        """
        try:
            print(f"📧 [EmailService] Generating reply for email: {email_id}, user_plan: {user_plan}")
            ...
            if self.llm_service:
                ...
                print(f"✅ [EmailService] Generated reply: {len(reply_data.get('body', ''))} characters, mock: {reply_data.get('mock', False)}")
                return reply_data
            else:
                # Fallback reply
...
{(user_name or "Team")}""",
    ...
    async def get_active_email_accounts(self, session: AsyncSession = None):
        ...
        try:
            ...
            print(f"❌ [EmailService] Error in get_active_email_accounts: {e}")
            return []
    ...
    async def get_pending_emails(self, session: AsyncSession = None, limit: int = 100):
        ...
        try:
            ...
            print(f"❌ [EmailService] Error in get_pending_emails: {e}")
            return []
    ...
    async def process_email_intelligence(self, email_id: str, session: AsyncSession = None) -> dict:
        ...
        try:
            ...
            print(f"❌ [EmailService] Error in process_email_intelligence: {e}")
            try:
                await db.rollback()
            except Exception:
                pass
    ...
    async def sync_account(self, account_id: str, session: AsyncSession = None) -> dict:
        ...
        try:
            ...
            print(f"❌ [EmailService] Error in sync_account: {e}")
            try:
                await db.rollback()
            except Exception:
                pass
