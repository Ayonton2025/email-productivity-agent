...
class SMTPService:
    ...
    async def send_email(
        ...
        if settings.ENABLE_MOCK_MODE:
            logger.info("Mock email accepted for delivery to %s", to)
            return (True, "Email accepted by offline mock delivery service")
        try:
            abuse_eval = None
            ...
            # Send via SMTP with bounded retries for transient provider failures.
            try:
                max_attempts = max(1, int(settings.SMTP_MAX_RETRIES))
                for attempt in range(1, max_attempts + 1):
                    try:
                        ...
                        if attempt >= max_attempts:
                            raise
                        logger.warning("SMTP attempt %s/%s failed; retrying", attempt, max_attempts)
    ...
    async def send_email_or_raise(self, *args, **kwargs) -> str:
        """Typed service boundary for callers that do not use legacy tuple results."""
        success, message = await self.send_email(*args, **kwargs)
        if not success:
    ...
    async def _append_to_sent(self, account: UserEmailAccount, msg: MIMEMultipart) -> bool:
        ...
        try:
            ...
            server = IMAPClient(account.imap_host, port=account.imap_port, use_uid=True, ssl=True)
