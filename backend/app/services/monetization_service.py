...
class AICreditService:
    ...
    @staticmethod
    async def check_credits_available(user_id: str, action: str, session: AsyncSession) -> bool:
    ...
    async def deduct_credits(
        user_id: str, action: str, model: str, session: AsyncSession, metadata: Dict = None
    ) -> Dict:
        """
    ...
    async def get_credits_balance(user_id: str, session: AsyncSession) -> Dict:
        ...
        subscription = await SubscriptionService.get_subscription(user_id, session)
        if not subscription:
            return {"balance": 0, "allocated": 0, "used": 0, "available": 0}
...
class OutboundEmailService:
    ...
    @staticmethod
    async def check_outbound_available(user_id: str, count: int, session: AsyncSession) -> bool:
...
class FeatureGatingService:
    ...
    @staticmethod
    async def can_access_feature(user_id: str, feature: str, session: AsyncSession) -> Dict:
        """
        ...
        subscription = await SubscriptionService.get_subscription(user_id, session)
        if not subscription:
            return {"allowed": False, "reason": "No subscription", "current_tier": None}
...
class EnterpriseBillingService:
    ...
    @staticmethod
    async def get_enterprise_modules(user_id: str, session: AsyncSession) -> List[Dict]:
        """Get active enterprise modules for user"""
        query = select(EnterpriseAddOn).where(
            (EnterpriseAddOn.user_id == user_id) & (EnterpriseAddOn.status == "active")
        )
        result = await session.execute(query)
...
class BillingReportService:
    ...
    @staticmethod
    async def generate_monthly_snapshot(user_id: str, session: AsyncSession):
        ...
        addon_query = select(OutboundAddOn).where(
            (OutboundAddOn.user_id == user_id) & (OutboundAddOn.status == "active")
        )
