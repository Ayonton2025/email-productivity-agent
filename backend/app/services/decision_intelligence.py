...
class DecisionIntelligenceService:
    ...
    async def extract_commitments(self, email: Email, email_content: str) -> List[Dict[str, Any]]:
...
Return only valid JSON array."""
            ...
            logger.error(f"⚠️ [DecisionIntelligence] Failed to extract commitments: {e}")
            return []

    async def extract_risks(self, email: Email, email_content: str) -> List[Dict[str, Any]]:
...
Return only valid JSON array."""
            ...
            logger.error(f"⚠️ [DecisionIntelligence] Failed to extract risks: {e}")
            return []

    async def extract_opportunities(self, email: Email, email_content: str) -> List[Dict[str, Any]]:
...
Return only valid JSON array."""
            ...
            logger.error(f"⚠️ [DecisionIntelligence] Failed to extract opportunities: {e}")
            return []
    ...
    async def get_upcoming_deadlines(self, user_id: str, days_ahead: int = 7) -> List[Commitment]:
    ...
    async def get_active_risks(self, user_id: str, severity: Optional[str] = None) -> List[Risk]:
        """Get active risks, optionally filtered by severity"""
        query = select(Risk).where(and_(Risk.user_id == user_id, Risk.status == "open"))
