...
class SyncHistoryService:
    ...
    async def complete_sync(
        self, sync_id: str, emails_processed: List[str], sync_status: str = "completed"
        ...
        try:
            ...
            result = await self.db.execute(select(SyncHistory).where(SyncHistory.id == sync_id))
    ...
    async def record_sync_error(self, sync_id: str, error_message: str) -> Optional[SyncHistory]:
        """
        Record a sync error.
    ...
    async def get_sync_history(self, user_id: str, limit: int = 50) -> List[SyncHistory]:
        """
