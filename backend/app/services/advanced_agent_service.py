...
class AdvancedAgentService:
    ...
    async def get_email_insights(self, time_range: str = "week") -> Dict[str, Any]:
        ...
        insights = await self.llm_service.advanced_email_analysis(
            {"emails": [email.to_dict() for email in emails]}, "cross_email_insights"
        )
    ...
    async def generate_contextual_reply(
        ...
        reply = await self.llm_service.generate_smart_reply(
            email.to_dict(), tone, [e.to_dict() for e in related_emails]
        )
    ...
    async def _store_analyzed_email(
        ...
        try:
            ...
            result = await self.db.execute(select(Email).where(Email.id == email_data.get("id")))
            ...
            logger.error(f"Error storing analyzed email: {e}")
