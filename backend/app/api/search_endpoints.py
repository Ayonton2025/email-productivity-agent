    ...
    try:
        ...
        if elasticsearch_service.enabled:
            ...
            result = await elasticsearch_service.search(
                user_id=current_user.id, query=q, limit=limit, offset=offset, filters=filters
            )
        ...
        # Get total count
        count_result = await db.execute(select(Email).where(Email.user_id == current_user.id))
...
async def get_search_suggestions(
    ...
    try:
        ...
        subject_result = await db.execute(
            ...
            .where(and_(Email.user_id == current_user.id, Email.subject.ilike(search_pattern)))
            .distinct()
            .limit(limit)
        )
        ...
        sender_result = await db.execute(
            select(Email.sender)
            .where(and_(Email.user_id == current_user.id, Email.sender.ilike(search_pattern)))
            .distinct()
            .limit(limit)
        )
        ...
        category_result = await db.execute(
            ...
            .where(and_(Email.user_id == current_user.id, Email.ai_category.ilike(search_pattern)))
            .distinct()
            .limit(limit)
        )
        ...
        return {"subjects": subjects, "senders": senders, "categories": categories}
