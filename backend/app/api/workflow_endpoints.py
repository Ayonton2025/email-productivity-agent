...
async def create_workflow(
    ...
    try:
        ...
        can_access = await gating_service.can_access_feature(
            user_id=current_user.id, feature="workflow_automation", session=db
        )
        if not can_access:
            raise HTTPException(
                status_code=403,
...
async def update_workflow_step(
    ...
    try:
        result = await db.execute(
            select(WorkflowStep)
            .join(Workflow)
            .where(and_(WorkflowStep.id == step_id, Workflow.user_id == current_user.id))
        )
...
async def delete_workflow_step(
    step_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
