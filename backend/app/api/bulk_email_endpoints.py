...
async def bulk_mark_read(
    ...
    try:
        ...
        result = await db.execute(
            select(Email).where(Email.id.in_(request.email_ids), Email.user_id == current_user.id)
        )
...
async def bulk_delete(
    ...
    try:
        ...
        if request.soft_delete:
            ...
            await db.execute(select(Email).where(Email.id.in_(request.email_ids)))
            for email in emails:
        ...
        return {"status": "success", "deleted_count": len(request.email_ids)}
