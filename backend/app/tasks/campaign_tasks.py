            ...
            for campaign in campaigns:
                try:
                    ...
                    lead_result = await session.execute(
                        select(Lead)
                        .where(and_(Lead.campaign_id == campaign.id, Lead.status == "pending"))
        ...
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(
                select(WarmupSchedule).where(
                    and_(WarmupSchedule.day_of_week == today, WarmupSchedule.is_active == True)
                )
            )
                ...
                try:
                    ...
                    for lead in leads:
                        ...
                        if campaign:
                            ...
                            if not account:
                                logger.warning(
                                    f"No active email account for campaign {campaign.id}; skipping lead {lead.id}"
                                )
                                continue
